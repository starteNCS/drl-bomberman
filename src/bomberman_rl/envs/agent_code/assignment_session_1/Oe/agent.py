...
class Agent:
    ...
    def act(self, state: dict, **kwargs) -> int:
        ...
        walls = state["walls"]
        crates = state["crates"]
        # Get individual crate locations
        crate_coordinates = np.nonzero(crates == 1)
        crate_coordinates = list(zip(crate_coordinates[0], crate_coordinates[1]))
        ...
        if bombs_left == True and (crates[np.clip(self_pos_coordinates[0] + 1, 0, 17), self_pos_coordinates[1]] == True or crates[np.clip(self_pos_coordinates[0] - 1, 0, 17), self_pos_coordinates[1]] == True or crates[self_pos_coordinates[0], np.clip(self_pos_coordinates[1] + 1, 0, 17)] == True or crates[self_pos_coordinates[0], np.clip(self_pos_coordinates[1] - 1, 0, 17)] == True):
            return Actions.BOMB.value #bomba

        if len(crates) != 0:
            closest_crate = (9999,9999) # hier bleibt der ganz evt stehen egi geht aber nicht weil
            # vorher sichergestellt dass mindestens ein coin auf der map
            closest_crate_distance = 99999
            for crate in crate_coordinates:
                curr_crate_distance = self.l1_distance(self_pos_coordinates, crate)
                if curr_crate_distance < closest_crate_distance or closest_crate is None:
                    closest_crate_distance = curr_crate_distance
                    closest_crate = crate
                    ## print(closest_coin)
            if closest_crate != (9999,9999):
                # print("hallo3")
                non_traversable_fields[closest_crate] = 0
                path_to_crate = self.can_reach_safe_spot(non_traversable_fields, self_pos_coordinates, closest_crate)
                # print(path_to_crate)
                if path_to_crate is not None:
                    optimal_next_field = np.asarray(path_to_crate[1])
                    # print("hallo4")
                    return self.turn_optimal_field_into_action(self_pos_coordinates, optimal_next_field)
                elif path_to_crate is None:
                    # print("bomboclat")
                    return Actions.WAIT.value
            else:
                # print("hallo2")
                ...
                chosen_action = Actions.WAIT.value
