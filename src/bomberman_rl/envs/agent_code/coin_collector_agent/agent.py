...
class Agent(Base):
    ...
    def act(self, game_state, **kwargs):
        game_state = gym2legacy(game_state)
        ...
        for (xb, yb), t in bombs:
            for i, j in [(xb + h, yb) for h in range(-3, 4)] + [
                (xb, yb + h) for h in range(-3, 4)
            ]:
        ...
        if d == (x, y) and (
            [arena[x + 1, y], arena[x - 1, y], arena[x, y + 1], arena[x, y - 1]].count(1)
            > 0
        ):
            action_ideas.append("BOMB")
